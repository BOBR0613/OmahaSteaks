...
public class Common {
	...
	public static void selectByBuiltIn(ComboBox cbb, String value, boolean isCloseDialog) {
		...
		} catch (ElementNotInteractableException notInvisible) {
			throw notInvisible;
...
//		System.out.print("] - " + jsStatus.concat(setThousandsSeparatorInTime(endTime)) + "\n");
	...
	public static WebDriver navigateToOmastkSiteWithExistingBrowserProfile(String userDir) {
		...
		if (Common.MODE.getRunningMode().equals(Constants.PLATFORM_DESKTOP)) driver.manage().window().maximize();
	...
	public static String getRewardSkuDescription(String value) {
		...
		if (rewardDesc.contains(")")) {
		  String tmpstr = rewardDesc.substring(rewardDesc.indexOf(")") + 1);
		  if (tmpstr.equalsIgnoreCase("")) rewardDesc = rewardDesc.substring(0,rewardDesc.indexOf("(") - 1).trim();
		  else rewardDesc = rewardDesc.substring(rewardDesc.indexOf(")") + 1).trim();
		}
