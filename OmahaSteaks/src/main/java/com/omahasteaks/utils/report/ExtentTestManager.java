package com.omahasteaks.utils.report;
...
public class ExtentTestManager {
	...
	public static synchronized ExtentTest startTest(String testName, String desc, ExtentTest parent) {
		try {
			if (ExtentManager.isReportCreated())
				...
				outputFolder = "test-output" + System.getProperty("file.separator")
						+ "Report-".concat(testName).concat("-").concat(Common.getNowTime("MM.dd.yyyy-HH.mm.ss"));
	...
	public static synchronized String getOutputFullPath() {
		return workingDir + System.getProperty("file.separator") + outputFolder;
	}
}