...
public class TC_RCP_006_UserCanRemoveRewardCardInPaymentReviewPageThenCompleteCheckout extends TestBase_2SC {
    @Inject
    ...
    @Test
    public void TC_RCP_006_User_Can_Remove_Reward_Card_In_Payment_Review_Page_Then_Complete_Checkout() {
	...
	removeRewardCardInPaymentReviewPage();

	verifyRewardCardIsRemovedInPaymentReviewPage();
    ...
    private void verifyRewardCardIsRemovedInPaymentReviewPage() {
	Logger.verify("In Payment & Review Page: " + "     Verify  the reward card is removed ");
	assertFalse(paymentAndReviewPage2SC.isRewardCardDisplayed(Constants.REWARD_CARD_PRICE_30), "the Reward card does not remove");
    }

    private void removeRewardCardInPaymentReviewPage() {
	Logger.info("In Payment and Review page:" + "In Gift Cards & Vouchers section:" + "- Click \"Remove\" link");
	paymentAndReviewPage2SC.removeRewardCard(Constants.REWARD_CARD_PRICE_30);
    }
	...
	assertTrue(paymentAndReviewPage2SC.isRewardCardDisplayed(Constants.REWARD_CARD_PRICE_30), "Reward Card does not display in Payment And Review Page");
    }
	...
	Logger.tc("TC_RCP_006 - User can remove Reward Card to Payment & Review Page then complete checkout");
	Logger.to("TO_RCP_06 - User can remove Reward Card in Payment & Review Page then complete checkout");
