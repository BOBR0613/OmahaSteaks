...
public class TC_SOP_006_UserCanCompleteCheckoutWithRushDelivery extends TestBase_2SC {
    ...
    @Test
    public void TC_SOP_006_User_Can_Complete_Checkout_With_Rush_Delivery_Shipping() {
	initTestCaseData();
	...
	if (paymentAndReviewPage2SC.isShippingMethodDisplaysOnPopup(myShippingMethod)) {
	    ...
	    Logger.warning("There is no \"Rush Delivery\" shipping options in CST time");
	}
    }
	...
	Logger.tc("TC_SOP_006 - User Can Complete Checkout With Rush Delivery Shipping");
	Logger.to("TO_SOP_23 - In Payment & Review page, user can click 'View All Shipping Options' link and select Shipping Method \"Rush Delivery\" on the 'Shipping & Delivery' modal then complete checkout");
	myAddress = lstAddresses.getRushShippingAddress();
	myItem.initRandom(Recipient.MYSELF);
	myShippingMethod = Constants.RUSH_SHIPPING;
    }
